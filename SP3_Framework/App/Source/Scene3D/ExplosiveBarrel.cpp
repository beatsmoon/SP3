 ...
 */
CExplosiveBarrel3D::CExplosiveBarrel3D(void)
	: cGroundMap(NULL)
	, iBarrelHP(100)
{
 ...
 */
CExplosiveBarrel3D::CExplosiveBarrel3D(const glm::vec3 vec3Position, CEntity3D::TYPE type, const glm::vec3 vec3Front, const float fYaw, const float fPitch)
	: cGroundMap(NULL)
	, iBarrelHP(100)
{
	// Set the default position to the origin
 ...
 */
bool CExplosiveBarrel3D::Init(void)
{
	// Check if the shader is ready
	if (!cShader)
	{
		cout << "CExplosiveBarrel3D::Init(): The shader is not available for this class instance." << endl;
		return false;
	}
	...
	case EXPLOSIVE_BARREL:
	{
		...
		vec3ColliderScale = glm::vec3(1.0, 1.0, 1.0); // collider scale
		// load structureMesh OBJ
		structureMesh = MeshBuilder::GenerateOBJ("Explosive_Barrel", "OBJ/TrainingBot.obj");
		// load and create a texture 
 ...
 */
void CExplosiveBarrel3D::SetModel(glm::mat4 model)
{
	this->model = model;
}
 ...
 */
void CExplosiveBarrel3D::SetView(glm::mat4 view)
{
	this->view = view;
}
 ...
 */
void CExplosiveBarrel3D::SetProjection(glm::mat4 projection)
{
...
*/
void CExplosiveBarrel3D::Update(const double dElapsedTime)
{
}
...
*/
void CExplosiveBarrel3D::ActivateCollider(Shader* cLineShader)
{
...
*/
void CExplosiveBarrel3D::PreRender(void)
{
	// Draw this as last
	glDepthFunc(GL_LEQUAL);  // change depth function so depth test passes when values are equal to depth buffer's content
}

void CExplosiveBarrel3D::RenderMesh(Mesh* mesh)
{
...
*/
void CExplosiveBarrel3D::Render(void)
{
	// If the shader is in this class, then do not render
	if (!cShader)
	{
		cout << "CExplosiveBarrel3D::Render(): The shader is not available for this class instance." << endl;
		return;
	}
...
*/
void CExplosiveBarrel3D::PostRender(void)
{
