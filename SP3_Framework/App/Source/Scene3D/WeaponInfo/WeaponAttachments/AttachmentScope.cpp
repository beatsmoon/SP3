 ...
 */
CGunScope::CGunScope(	const glm::vec3 vec3Position,
 ...
 */
void CGunScope::SetModel(glm::mat4 model)
{
	this->model = model;
}
 ...
 */
void CGunScope::SetView(glm::mat4 view)
{
	this->view = view;
}
 ...
 */
void CGunScope::SetProjection(glm::mat4 projection)
{
...
*/
void CGunScope::Update(const double dElapsedTime)
{
	//cPlayer3D->GetWeapon()->GetScope()->GetTierLevel();
	switch (eTierLevel)
	{
	case TIERLEVEL::NOTIER:
	{
...
*/
void CGunScope::ActivateCollider(Shader* cLineShader)
{
...
*/
void CGunScope::Render(void)
{
	// If the shader is in this class, then do not render
	if (!cShader)
	{
		cout << "CGunScope::Render(): The shader is not available for this class instance." << endl;
		return;
	}
...
*/
void CGunScope::PostRender(void)
{
...
void CGunScope::SetTierLevel(const TIERLEVEL eStatus)
{
	eTierLevel = eStatus;
}
CGunScope::TIERLEVEL CGunScope::GetTierLevel(void) const
{
	return eTierLevel;
}