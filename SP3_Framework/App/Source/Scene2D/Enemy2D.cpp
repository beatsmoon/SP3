...
bool CEnemy2D::Init(void)
{
	...
	if (LoadTexture("Images/enemy.tga") == false)
	{
...
void CEnemy2D::Update(const double dElapsedTime)
{
	...
	case PATROL:
		...
		if (cPhysics2D.CalculateDistance(i32vec2Index, cPlayer2D->Geti32vec2Index()) < 2.0f && cPlayer2D->GetIsInvissible() == false)
		{
			sCurrentFSM = ATTACK;
			iFSMCounter = 0;
		}
...
#endif
		if (cPhysics2D.CalculateDistance(i32vec2Index, cPlayer2D->Geti32vec2Index()) < 2.0f)
		{
			// Attack
...
void CEnemy2D::SetIsActive(bool isactive)
{
	isActive = isactive;
}

bool CEnemy2D::GetIsActive()
{
	return isActive;
}
...
bool CEnemy2D::InteractWithPlayer(void)
{
	...
	if (((i32vec2Index.x >= i32vec2PlayerPos.x - 0.5) && 
		...
		(i32vec2Index.y <= i32vec2PlayerPos.y + 0.5))
		&& cPlayer2D->GetIsInvissible() == false)
	{

		cPlayer2D->SetHealth((cPlayer2D->GetHealth() - 1));
		cout << "Gotcha!" << endl;
