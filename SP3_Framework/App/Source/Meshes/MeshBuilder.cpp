...
Mesh* MeshBuilder::GenerateAxes(const std::string &meshName, float lengthX, float lengthY, float lengthZ)
{
	...
	v.pos = glm::vec3(-1000, 0, 0);
	v.color = glm::vec3(1, 0, 0);
	vertex_buffer_data.push_back(v);
	v.pos = glm::vec3(1000, 0, 0);
	v.color = glm::vec3(1, 0, 0);
	vertex_buffer_data.push_back(v);
	v.pos = glm::vec3(0, -1000, 0);
	v.color = glm::vec3(0, 1, 0);
	vertex_buffer_data.push_back(v);
	v.pos = glm::vec3(0, 1000, 0);
	v.color = glm::vec3(0, 1, 0);
	vertex_buffer_data.push_back(v);
	v.pos = glm::vec3(0, 0, -1000);
...
Mesh* MeshBuilder::GenerateCrossHair(const std::string &meshName, float colour_r, float colour_g, float colour_b, float length)
{
	...
	std::vector<Vertex> vertex_buffer_data;
	// Vertex #1
	v.pos = glm::vec3(-length, 0, 0);
	v.color = glm::vec3(colour_r, colour_g, colour_b);
	vertex_buffer_data.push_back(v);
	// Vertex #2
	v.pos = glm::vec3(length, 0, 0);
	v.color = glm::vec3(colour_r, colour_g, colour_b);
...
Mesh* MeshBuilder::GenerateQuad(const std::string &meshName, glm::vec3 color, float length)
{
	...
	v.pos = glm::vec3(-0.5f * length,-0.5f * length,0);
	v.color = color;
...
Mesh* MeshBuilder::GenerateRing(const std::string &meshName, glm::vec3 color, unsigned numSlice, float outerR, float innerR)
{
	...
	for(unsigned slice = 0; slice < numSlice + 1; ++slice)
	{
		...
		v.color = color;
		v.pos = glm::vec3(outerR * cos(Math::DegreeToRadian(theta)), 0, outerR * sin(Math::DegreeToRadian(theta)));
		vertex_buffer_data.push_back(v);
		
		v.color = color;
		v.pos = glm::vec3(innerR * cos(Math::DegreeToRadian(theta)), 0, innerR * sin(Math::DegreeToRadian(theta)));
		vertex_buffer_data.push_back(v);
	}
	for(unsigned slice = 0; slice < numSlice + 1; ++slice)
	{
...
Mesh* MeshBuilder::GenerateSphere(const std::string &meshName, glm::vec3 color, unsigned numStack, unsigned numSlice, float radius)
{
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	for(unsigned stack = 0; stack < numStack + 1; ++stack) //stack //replace with 180 for sphere
	{
		...
		for(unsigned slice = 0; slice < numSlice + 1; ++slice) //slice
		{
			float theta = slice * degreePerSlice;
			Vertex v;
			v.pos = glm::vec3(radius * sphereX(phi, theta), radius * sphereY(phi, theta), radius * sphereZ(phi, theta));
			v.color = color;
			vertex_buffer_data.push_back(v);
		}
	}
