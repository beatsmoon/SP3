...
public:
	...
	static Mesh* GenerateRing(const std::string &meshName, glm::vec3 color, unsigned numSlice, float outerR = 1.f, float innerR = 0.f);
	static Mesh* GenerateSphere(const std::string &meshName, glm::vec3 color, unsigned numStack, unsigned numSlice, float radius = 1.f);
